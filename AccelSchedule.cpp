...
// 2. wt_mem is large enough to hold the weights for at least 1 output feature map
// -----------------------------------------------------------------------
void compute_accel_schedule(
	...
	// imgs_per_batch is the number of output feature maps to compute per batch
	...
	for (unsigned o = 0; o < n_outputs; o+=imgs_per_batch, idx++) {
		...
		/* print accelInfo */
		printf("schedule [%d]: wt_offset = [%d], kh_offset = [%d], n_inputs = [%d], n_outputs = [%d], layer_mode = [%d], width_mode = [%d], norm_mode = [%d]\n", (int)idx, (int)o, (int)o, (int)(schedule[idx].n_inputs), (int)(schedule[idx].n_outputs), (int)(schedule[idx].layer_mode), (int)(schedule[idx].width_mode), (int)(schedule[idx].norm_mode));
	}
...
// -----------------------------------------------------------------------
unsigned find_conv_batch_size(unsigned width, unsigned width_o, unsigned n_inputs, unsigned n_outputs){
...
void load_conv1_weights(Word* wt, Word* wt_o, unsigned o, unsigned n_out)
{
	...
	for (unsigned n = 0; n < n_out; ++n) {
		for (unsigned i = 0; i < M; ++i) {
			...
			w_o = w_o >> WT_SIZE;
			w_o(M*WT_SIZE-1, (M-1)*WT_SIZE) = w(WT_SIZE-1, 0);
			w = w >> WT_SIZE;
...
// -----------------------------------------------------------------------
void load_conv_weights(Word* wt, Word* wt_o, unsigned o, unsigned n_in, unsigned n_out) {
	// curr is the index of the starting weight in [wt]
	...
	for (unsigned n = 0; n < (wt_words+CONVOLVERS-1)/CONVOLVERS; ++n) {
		for (unsigned i = 0; i < CONV_W_PER_WORD*CONVOLVERS; ++i) {
			...
			w_o[i % CONVOLVERS](CONV_W_PER_WORD*WT_SIZE-1, (CONV_W_PER_WORD-1)*WT_SIZE) = w(WT_SIZE-1, 0);
			w = w >> WT_SIZE;
...
// -----------------------------------------------------------------------
void load_dense_weights(Word* wt, Word* wt_o, unsigned o, unsigned n_in, unsigned n_out) {
