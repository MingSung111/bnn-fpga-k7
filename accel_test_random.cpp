...
void test_conv_layer_random(
  ...
  for (unsigned m = 0; m < DMEM_WORDS; m++){
	  for (unsigned i = 0; i < WORD_SIZE; i++){
		  fprintf(fp_data_in, "%d", (int)data_i[m][i]);
	  }
	  fprintf(fp_data_in, "\n");
  }
  ...
  for (unsigned m = 0; m < S * S; m++){
	  for (unsigned i = 0; i < WORD_SIZE; i++){
		  fprintf(fp_conv_ref, "%d", (int)conv_ref[m][i]);
	  }
	  fprintf(fp_conv_ref, "\n");
  }
  ...
  for (unsigned m = 0; m < S * S; m++){
	  for (unsigned i = 0; i < WORD_SIZE; i++){
		  fprintf(fp_bin_ref, "%d", (int)bin_ref[m][i]);
	  }
	  fprintf(fp_bin_ref, "\n");
  }
...
int main() {
	...
	fp_data_in = fopen("data_in.txt", "w");
	fp_data_out = fopen("data_out.txt", "w");
	fp_wt = fopen("wt.txt", "w");
	fp_kh = fopen("kh.txt", "w");
	fp_conv_ref = fopen("conv_ref.txt", "w");
	fp_bin_ref = fopen("bin_ref.txt", "w");
