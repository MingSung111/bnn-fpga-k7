...
void bin_conv(
	...
	// ---------------------------------------------------------------------
	LOOP_PHASES: for (ap_uint<10> p = 0; p < n_phases; p += images_per_phase) {
		DB(3, printf ("=== PHASE %d ===\n", p.to_int()) );
		...
		LOOP_WORDS_IN_PHASE: for (ap_uint<8> count = 0; count < WORDS_PER_PHASE+images_per_phase; ++count) {
			...
			if (wrd == 0) {
				...
				// -------------------------------------------------------------------
				LOOP_WT_WORDS: for (IdxType m = 0; m < CONVOLVERS; ++m) {
					/*if (wt_offset == 0)
				...
				// -------------------------------------------------------------------
				LOOP_LOAD_WTS: for (IdxType m = 0; m < CONVOLVERS; ++m) {
					for (ap_uint<2> kr = 0; kr < K; ++kr) {
						for (ap_uint<2> kc = 0; kc < K; ++kc)
			...
			if (wrd != words_per_image) {
				LOOP_CONVOLVER_LOAD: for (IdxType m = 0; m < CONVOLVERS; ++m) {
			...
			// Compute
			LOOP_CONVOLVERS: for (IdxType m = 0; m < CONVOLVERS; ++m) {
	...
	LOOP_ACC_PHASES: for (ap_uint<5> w = 0; w < words_per_image; ++w) {
		for (IdxType b = 0; b < WORD_SIZE; ++b) {
      	  #pragma HLS unroll
			fixed_temp[b] = fixed_buffer[w][b];
		}

		LOOP_ACC_PHASES_I: for (ap_uint<8> i = words_per_image; i < WORDS_PER_PHASE; i += words_per_image) {
			for (IdxType b = 0; b < WORD_SIZE; ++b) {
	...
	DB(4, unsigned width = 1 << log_width; printf ("=== conv result ===\n"); print_mat(fixed_buffer[0], width, 8, width););
...
// -----------------------------------------------------------------------
void fp_conv(
		Word wt_mem[CONVOLVERS][C_WT_WORDS], // wt_mem is also partitioned into two banks for each convolver, and interleaved
		Word kh_mem[KH_WORDS],
		Word dmem[2][CONVOLVERS][C_DMEM_WORDS], // dmem[0]/dmem[1], one for input, one for output
		ap_uint<1> d_i_idx,
		ap_uint<1> d_o_idx,
		const Address kh_index,
		const Address o_index,
		const unsigned N // # of output fmaps
) {

	const unsigned M = 3; // # of input fmaps (channels)
	const unsigned S = 32; // # of input dim
	const unsigned OUTWORDS = S * S / WORD_SIZE; // words per output image

	C1InputType win[M][K][K]; // input sliding window
	C1InputType lbuf[M][K-1][S]; // line buffer, parallel across M, line_buffer_size != window_size
	...
	LOOP_FP_CONV_O: for (IdxType n = 0; n < N; ++n) {

		// clear line buffers for each new output map
		LOOP_RESET_LINEBUFFERS: for (IdxType m = 0; m < M; ++m) {
		...
		LOOP_LOAD_WTS: for (ap_uint<2> m = 0; m < M; ++m) {
			wtbuf[m] = wt_word((m + 1) * WT_SIZE - 1, m * WT_SIZE);
			//Modified by Yu - 2017.10.23
			//print_wt(wtbuf[m]);
			printf("--\n");
		}
		...
		// begin convolution
		LOOP_CONV_ROWS: for (IdxType r = 0; r < S + 1 /*starting index of the window is upper-left corner*/; ++r) {
			LOOP_CONV_COLS: for (IdxType c = 0; c < S + 1; ++c) {
				...
				for (ap_uint<2> m = 0; m < M; ++m) {
					...
					pix(W - 1, 0) = inword((m+1) * W - 1, m * W); // each input word contains three inputs for three different channels, parallel across channel
				...
				if (r > 0 && c > 0) {
					...
					outwords[(r - 1) / 2][((r - 1) % 2) * S + (c - 1)] = (res >= nc) ? Bit(0) : Bit(-1);
				}
		...
		LOOP_OUTPUT: for (IdxType i = 0; i < OUTWORDS; ++i) {
...
void bin_dense(
		...
		ap_uint<1> d_o_idx,
		const Address o_index,
		const unsigned n_inputs, // # of input bits
		const unsigned n_outputs // # of output bits
) {
	...
	LOOP_DENSE_O: for (Address o = 0; o < n_outputs; ++o) {
		...
		LOOP_DENSE_I: for (Address i = 0; i < n_inputs; i += CONVOLVERS * WORD_SIZE) {
...
void top(
	...
	static Word dmem[2][CONVOLVERS][C_DMEM_WORDS];//dmem[0]/dmem[1], one for input, one for output
	...
	if (layer_type == LAYER_CONV1) {
		...
		LOOP_IMG_BATCH: for (IdxType i = 0; i < n_outputs; ++i) {
